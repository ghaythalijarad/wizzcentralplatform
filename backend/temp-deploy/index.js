...
async function handleAgentConnect(connectionId, message, apiGatewayClient) {
    ...
    try {
        ...
        // Get active chat sessions for this agent to sync (limit to recent sessions to avoid payload size issues)
        const activeSessions = await getActiveChatSessions();
        const recentSessions = activeSessions.slice(0, 10); // Limit to 10 most recent sessions
        
        // Send initial session sync with limited data
        await sendToConnection(connectionId, {
            action: 'sessions_sync',
            sessions: recentSessions.map(session => ({
                sessionId: session.sessionId,
                userDisplayName: session.userDisplayName || session.driverName,
