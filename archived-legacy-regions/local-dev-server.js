...
let comprehensiveIraqiRegions = [];

// Try to load from Mapbox-generated file
const regionsFilePath = path.join(__dirname, 'data', 'comprehensive-iraq-regions.json');
try {
    if (require('fs').existsSync(regionsFilePath)) {
        comprehensiveIraqiRegions = JSON.parse(require('fs').readFileSync(regionsFilePath, 'utf8'));
        console.log(`✅ Loaded ${comprehensiveIraqiRegions.length} regions from Mapbox-generated data`);
        console.log(`   📊 Breakdown:`);
        console.log(`      - Country: ${comprehensiveIraqiRegions.filter(r => r.level === 'country').length}`);
        ...
        console.warn('⚠️  Mapbox-generated regions file not found. Run: node scripts/create-all-iraq-regions.js');
        console.warn('⚠️  Using fallback embedded data...');
    }
} catch (error) {
...
// ============================================
// REGIONS MANAGEMENT API - MAPBOX-POWERED SYSTEM
// ============================================

/**
 * Load regions from Mapbox-generated JSON file
 * Run: node scripts/create-all-iraq-regions.js to generate
 */
let comprehensiveIraqiRegions = [];
const regionsFilePath = path.join(__dirname, 'data', 'comprehensive-iraq-regions.json');
...
try {
    ...
    if (fs.existsSync(regionsFilePath)) {
        comprehensiveIraqiRegions = JSON.parse(fs.readFileSync(regionsFilePath, 'utf8'));
        console.log('\n🗺️  MAPBOX REGIONS LOADED:');
        console.log(`   ✅ Total Regions: ${comprehensiveIraqiRegions.length}`);
        console.log(`   📊 Breakdown:`);
        console.log(`      - Country: ${comprehensiveIraqiRegions.filter(r => r.level === 'country').length}`);
        console.log(`      - Governorates: ${comprehensiveIraqiRegions.filter(r => r.level === 'governorate').length}`);
        console.log(`      - Districts: ${comprehensiveIraqiRegions.filter(r => r.level === 'district').length}`);
        console.log(`      - Neighborhoods: ${comprehensiveIraqiRegions.filter(r => r.level === 'neighborhood').length}`);
        console.log(`   📁 Source: ${regionsFilePath}\n`);
    } else {
        console.warn('\n⚠️  WARNING: Mapbox regions file not found!');
        console.warn(`   Expected: ${regionsFilePath}`);
        console.warn(`   Run: node scripts/create-all-iraq-regions.js`);
    ...
    // NAJAF DISTRICTS - ENHANCED WITH GADM BOUNDARIES (4 DISTRICTS)
    {
        id: 'najaf_central',
        ...
        delivery_config: { base_fee: 2500, per_km_fee: 500, minimum_order: 15000, free_delivery_threshold: 50000, estimated_time_minutes: 35 }
    },
    {
        id: 'najaf_kufa',
    ...
    },
    {
        ...
        delivery_config: { base_fee: 3000, per_km_fee: 700, minimum_order: 20000, free_delivery_threshold: 60000, estimated_time_minutes: 50 }
    },
    {
        id: 'najaf_mishkhab',
        ...
        delivery_config: { base_fee: 3000, per_km_fee: 650, minimum_order: 18000, free_delivery_threshold: 55000, estimated_time_minutes: 45 }
    },
    ...
    // NAJAF NEIGHBORHOODS - COMPREHENSIVE 16 NEIGHBORHOODS

    // NAJAF CENTRAL DISTRICT NEIGHBORHOODS (6 neighborhoods)
    {
        id: 'najaf_old_city',
        name: 'Old City Najaf',
    ...
    // AL-KUFA DISTRICT NEIGHBORHOODS (4 neighborhoods)
    {
        id: 'kufa_center',
        name: 'Kufa Center',
        name_ar: 'مركز الكوفة',
    ...
    // AL-MANATHERA DISTRICT NEIGHBORHOODS (3 neighborhoods)
    {
        id: 'manathera_center',
    ...
    // AL-MISHKHAB DISTRICT NEIGHBORHOODS (3 neighborhoods)
    {
        id: 'mishkhab_center',
    ...
    // END NAJAF COMPREHENSIVE NEIGHBORHOODS
