...
class RegionsManager {
    ...
    async loadRegions() {
        ...
        try {
            ...
            // ALWAYS try to load from backend - never use sample data unless API completely fails
            console.log('📊 Attempting to fetch from API...');
            const backendRegions = await this.fetchRegionsFromBackend();
            
            console.log('📊 fetchRegionsFromBackend returned:', {
                isNull: backendRegions === null,
                isArray: Array.isArray(backendRegions),
                length: backendRegions ? backendRegions.length : 'N/A'
            });
            
            if (backendRegions && Array.isArray(backendRegions) && backendRegions.length > 0) {
                console.log(`✅ SUCCESS! Loaded ${backendRegions.length} regions from API`);
                ...
                console.log(`✅ UI updated with ${this.regions.length} regions`);
            } else {
                console.error('❌ API returned invalid data - backendRegions:', backendRegions);
                throw new Error('API returned empty or invalid data');
            }
            
        } catch (error) {
            console.error('❌ ========== LOAD REGIONS FAILED ==========');
            console.error('❌ Error:', error.message);
            console.error('❌ Stack:', error.stack);
            
            // Fallback to sample data ONLY on complete failure
    ...
    async fetchRegionsFromBackend() {
        try {
            ...
            if (!response.ok) {
                console.log('🔄 First attempt failed, trying with authentication...');
            ...
            console.log('📊 Raw API Response structure:', {
                hasData: !!result.data,
                hasRegions: !!result.regions,
                isArray: Array.isArray(result),
                keys: Object.keys(result),
                dataLength: result.data?.length || result.regions?.length || (Array.isArray(result) ? result.length : 0)
            });
            ...
            if (result.data) {
                regions = result.data;
                console.log(`📊 Using result.data with ${regions.length} regions`);
            } else if (result.regions) {
                regions = result.regions;
                console.log(`📊 Using result.regions with ${regions.length} regions`);
            } else if (Array.isArray(result)) {
                regions = result;
                console.log(`📊 Using direct array with ${regions.length} regions`);
            } else {
                console.warn('⚠️ Unexpected API response format:', result);
            }
            
            if (regions.length === 0) {
                console.warn('⚠️ API returned 0 regions');
                return null;
            }
            ...
            const transformedRegions = regions.map(region => this.transformRegionData(region));
            console.log(`✅ Successfully transformed ${transformedRegions.length} regions`);
            console.log(`📊 Sample transformed region:`, transformedRegions[0]);
    ...
    transformRegionData(region) {
        ...
        let coordinates = { lat: 33.3152, lng: 44.3661 }; // Default Najaf coordinates
        if (region.coordinates) {
            coordinates = {
                lat: region.coordinates.lat || 33.3152,
                lng: region.coordinates.lng || 44.3661
            };
        }
        
        return {
            regionId: region.id || region.regionId,
            regionName: region.name || region.regionName,
            regionNameArabic: region.name_ar || region.regionNameArabic || '',
            governorate: region.governorate_id || region.governorate || 'Najaf',
            region_type: region.level === 'district' ? 'DISTRICT' : 'NEIGHBORHOOD',
            parent_id: region.parent_id,
            isActive: region.is_active !== false,
            status: (region.is_active !== false) ? 'ACTIVE' : 'INACTIVE',
            gps_coordinates: coordinates,
            coordinates: {
                center: coordinates,
                boundaries: region.boundary ? region.boundary.coordinates : []
            },
            serviceTypes: region.service_config || { delivery: true, pickup: false, dineIn: false },
            deliveryFee: region.delivery_config ? region.delivery_config.base_fee : 2500,
            minimumOrder: region.delivery_config ? region.delivery_config.minimum_order : 15000,
            estimatedDeliveryTime: region.delivery_config ? region.delivery_config.estimated_time_minutes : 30,
            activeDrivers: region.statistics ? region.statistics.active_drivers : 0,
            activeMerchants: 0,
            totalOrders: region.statistics ? region.statistics.total_orders : 0,
            level: region.level === 'district' ? 2 : 3,
            enhanced_with_gadm: region.enhanced_with_gadm || false,
            gadm_data: region.gadm_data || null
        };
    }
    ...
    getSampleRegions() {
        return [
            ...
            },
            {
                ...
                parent_id: 'REG_001', // Child of Baghdad Central
                isActive: true,
                status: 'ACTIVE',
                gps_coordinates: { lat: 33.3400, lng: 44.3400 },
                coordinates: {
    ...
    renderTableView() {
        ...
        // Render table rows - Simplified for regions management focus
    ...
    renderMapMarkers() {
        ...
        if (!this.map) {
            console.log('ℹ️ Map not initialized, skipping marker rendering');
            return;
        }
    ...
    showLoadingState() {
        ...
        if (!container) {
            console.error('❌ Cannot show loading state - regionsTableBody element not found');
            return;
        }
        container.innerHTML = `
            <tr>
    ...
    showDevModeIndicator() {
        // Find or create the dev mode indicator
        let indicator = document.querySelector('.dev-mode-indicator');
        if (!indicator) {
            indicator = document.createElement('div');
            indicator.className = 'dev-mode-indicator';
            indicator.innerHTML = `
                <i class="fas fa-code"></i>
                <span>Development mode: showing sample data</span>
            `;
