...
async function loadMerchantsFromDynamoDB() {
    try {
        ...
        // Use temporary credentials from Cognito session
        const accessToken = sessionStorage.getItem('accessToken');
        if (!accessToken) {
        ...
        const response = await fetchMerchantsFromAPI();
        
        if (response && response.businesses) {
            merchantsData = response.businesses.map(business => ({
                ...
                commission: business.commission || 0,
                ordersToday: business.ordersToday || 0,
                revenueToday: business.revenueToday || 0,
                rating: business.rating || null,
                joinDate: business.createdAt || business.registrationDate || 'N/A',
                avatar: business.logo || generateAvatarUrl(business.businessName || business.name),
                address: business.address || 'N/A',
                owner: business.owner || 'N/A'
            }));
        } else {
        ...
        console.log('Loaded', merchantsData.length, 'merchants');
        ...
        merchantsData = getSampleMerchantsData();
        filteredMerchants = [...merchantsData];
        showMessage('Using sample data - API connection failed', 'warning');
    }
}
...
async function fetchMerchantsFromAPI() {
    try {
        // This should be replaced with your actual API endpoint
        // For now, return mock data that simulates DynamoDB structure
        return {
            businesses: [
                {
                    businessId: 'biz-001',
...
async function updateMerchantStatus(merchantId, newStatus, reason = '') {
    try {
        ...
        // Here you would make an API call to update the status in DynamoDB
